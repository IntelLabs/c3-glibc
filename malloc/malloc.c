...
//#define CHECK_CHUNK_MEM_NO_ENCODED_PARAMS
...
mchunkptr mem2chunk(void *mem) {
  assert(!is_encoded_cc_ptr((uint64_t)mem));
...
musable (void *mem_in)
{
  ...
  if (is_encoded_cc_ptr((uintptr_t)mem_in)) {
    ...
    // purpose of determining the metadata size, but the usable space is sometimes larger than the
    // original requested size for the allocation. That may result in the reconstructed pointer
    // having a different power value and metadata size than the original pointer. In corner cases,
    // that may result in the computed usable space being less than the original requested size,
    // which could lead to functional errors in programs. Specifically, that would occur if the
    // difference between the usable space and the original requested size is less than the increase
    // in metadata size due to the aforementioned imprecision in computing the power field.
    mem = (void *)cc_decode_pointer((uintptr_t)mem_in);
    if (lim_enabled) {
      uint8_t enc_sz = get_encoded_size((uint64_t)mem_in);
  ...
  if (mem != 0)
    {
      ...
      if (__builtin_expect (using_malloc_checking == 1, 0))
        return malloc_check_get_size (p) - meta_size;

      if (chunk_is_mmapped (p))
	{
	  if (DUMPED_MAIN_ARENA_CHUNK (p))
	    return chunksize (p) - (SIZE_SZ + meta_size);
	  else
	    return chunksize (p) - ((2 * SIZE_SZ) + meta_size);
	}
      else if (inuse (p))
        return chunksize (p) - (SIZE_SZ + meta_size);
    }
...
{
#ifdef CC_DISABLE_MEM_STATS_FOR_THREADS
  (void) hit_new_peak;
#else // !CC_DISABLE_MEM_STATS_FOR_THREADS
...
#endif // !CC_DISABLE_MEM_STATS_FOR_THREADS
}
